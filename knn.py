...
# Se sinal era de venda (-1), retornos são invertidos
    ...
    for d in signal_knn.index:
        if signal_knn[crypto_string].loc[d]==1:
            returns_knn.set_value(d, crypto_string, (1-CT)*(df[crypto_string]['Adj Close'].loc[d]/df[crypto_string]['Adj Close'].loc[:d].iloc[-2]) - 1 - df_livre_risco['IRX Semanal'].loc[d])
        elif signal_knn[crypto_string].loc[d]==-1:
            returns_knn.set_value(d, crypto_string, (1-CT)*(df[crypto_string]['Adj Close'].loc[:d].iloc[-2]/df[crypto_string]['Adj Close'].loc[d]) -1 - df_livre_risco['IRX Semanal'].loc[d])
...
#    weights_ew = ew_weights       
    ...
    strat_index_knn_ew = pd.DataFrame(data={'Return': (ew_weights * returns_ew).dropna().sum(axis=1), 
                           'Level': np.nan})
...
#            ivp_weights.set_value(d, tracker, w[tracker]) 
    ...
    strat_index_knn_ivp = pd.DataFrame(data={'Return': (ivp_weights * returns_ivp).dropna().sum(axis=1), 
                           'Level': np.nan})
...
if roda_MVO==1:
    ...
    strat_index_knn_mvp = pd.DataFrame(data={'Return': (mvp_weights * return_mvp).dropna().sum(axis=1), 
                           'Level': np.nan})
